...
partileCharge_(iConfig.getParameter<int>("partileCharge")),
...
HITrackCorrectionAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   for(edm::View<reco::Track>::size_type i=0; i<tcol->size(); ++i){ 
     ...
     if(partileCharge_ == 1 && tr->charge()!=1) continue;
     if(partileCharge_ == -1 && tr->charge()!=-1) continue;
     if(partileCharge_ == 2 && tr->charge()==0) continue;
   ...
   {      
     ...
     if(partileCharge_ == 1 && tp->charge()!=1) continue;
     if(partileCharge_ == -1 && tp->charge()!=-1) continue;
     if(partileCharge_ == 2 && tp->charge()==0) continue;
